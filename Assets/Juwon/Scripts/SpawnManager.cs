...
public class SpawnManager : MonoBehaviour
{
    ...
    [SerializeField] GameObject UnitSpawn(int value, Transform pos)
    {
        if(getSpawnCounts[value] > defaultSpawnCounts[value]) return null; //정해진 수보다 많아지면 리턴
        
        GameObject unit = unitPool.Get(value);
        unit.transform.position = pos.position; //정해진 소환할 위치에 소환(우선 정함)
    ...
    [SerializeField] GameObject EnemySpawn(int value, Transform pos)
    {
